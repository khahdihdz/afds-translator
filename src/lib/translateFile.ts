...
export async function translateProjectFile(
  ...
  if (file.format === 'json') {
    const translated = await translateJson(file.originalContent, apiKey, settings, glossary, onProgress, signal)
  ...
  if (XML_LIKE.includes(file.format)) {
    const translated = await translateXml(file.originalContent, apiKey, settings, glossary, onProgress, signal)
  ...
  if (file.format === 'po') {
    const translated = await translatePo(file.originalContent, apiKey, settings, glossary, onProgress, signal)
    return { translatedContent: translated, warnings }
  }

  // Default: line-based generic text (txt, csv, yaml, ini, lua, renpy, js/ts/cs/java/kt, etc.)
  const result = await translatePlainText(file.originalContent, apiKey, settings, glossary, onProgress, signal)
